...
#ifdef CONFIG_QE_16_8_FIX
	data = ((u32) (UCC_GUEMR_SET_RESERVED3 | speed)) << 24;
	clrsetbits_be32((u32 *)guemr, 0x03000000, data);
#else
...
int ucc_set_tdm_rxtx_clk(int tdm_num, char *clk_src, enum comm_dir mode)
{
	...
	temp =  in_be32(&qe_mux_reg->cmxsi1cr_l) | clock_bits;
	out_be32(&qe_mux_reg->cmxsi1cr_l, temp);
...
int ucc_set_tdm_rxtx_sync(int tdm_num, char *sync_src, enum comm_dir mode)
{
	...
	temp = in_be32(&qe_mux_reg->cmxsi1syr) | clock_bits;
	out_be32(&qe_mux_reg->cmxsi1syr, temp);
