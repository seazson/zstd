...
static int aer_inject(struct aer_error_inj *einj)
{
	...
	int ret = 0;
	static struct aer_capability_regs regs;
	int severity;
	...
	//for APEI
	regs.uncor_status   = einj->uncor_status;
	regs.uncor_mask     = uncor_mask;
	regs.uncor_severity = sever;
	regs.cor_status     = einj->cor_status;
	regs.cor_mask       = cor_mask;
	regs.header_log.dw0 = einj->header_log0;
	regs.header_log.dw1 = einj->header_log1;
	regs.header_log.dw2 = einj->header_log2;
	regs.header_log.dw3 = einj->header_log3;
	regs.root_status    = rperr->root_status;
	regs.cor_err_source = rperr->source_id;
	regs.uncor_err_source = rperr->source_id;
	dev_warn(&dev->dev,
			 "aer_inject: UE=%x CE=%x Severity=%d\n",regs.uncor_status,regs.cor_status,severity);
	...
	if (find_aer_device(rpdev, &edev)) {
		...
		dev_err(&rpdev->dev, "aer_inject: AER device not found, Use APEI mode\n");
		aer_recover_queue(einj->domain, einj->bus, devfn, severity, &regs);
		//ret = -ENODEV;
	}
out_put:
	kfree(err_alloc);
