...
exec_dump (const char *exec)
{
  ...
  Dwfl *dwfl = pid_to_dwfl (pid); /*通过proc attach设备*/
  char *selfpathname;
  ...
  data.mod = NULL;
  dwfl_getmodules (dwfl, see_exec_module, &data, 0); /*遍历所有模块,找到可执行文件对应的mod*/
  assert (data.mod != NULL);
  GElf_Addr loadbase;
...
static const struct argp_option options[] =
  {
    { "backtrace-exec", OPT_BACKTRACE_EXEC, "EXEC", 0, "Run executable", 0 },
